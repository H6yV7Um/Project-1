...
exports.updateForIds = async ctx => {
    ...
    let departmentsInfo = [];
    const departments = await Request.post({department_ids : data.department_ids, access_token : global.coursera.ctx.cookies.get('access_token')}, API.DD_GET_DEPARTMENTS_INFO, API.HEADER);;
...
exports.get = async ctx => {
    ...
    if(data.isLeader)
    {
        condition.$or = [{department_id : {$in : global.coursera.user.leader_department_ids}}, {parent_department_ids : {$in : global.coursera.user.leader_department_ids}}];
    }
