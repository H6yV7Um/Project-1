...
class DepartmentList extends Component {
    ...
    formatOptionData(itemList, departmentUsers)
    {
        ...
        if(this.props.location.state.riao != 100)
        {
            for(let val of newUsers)
            {
                let color = "#35bfff"
                if(this.props.params.name != '高管')
                {
                    if(Executives.indexOf(val.name) == -1)
                    {
                        series_data.push({
                            value:0,
                            linHeight: '15px',
    ...
    {
        let newData = this.formatOptionData(itemList,departmentUsers),
            option = {}
        let yAxis_data = [...newData.yAxis_data];
                ...
                for(let val of newData.tooltipObj)
                {
                    if(val.name == obj.name)
                    {
                        ...
                        return '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 部门：' +
                            ...
                            val.name  + '<br>' +
                            '证书数量：' + val.count + '<br>' +
                            '部门平均：' + this.props.location.state.value + '<br>'
                    }
            }
        }}
        option.grid = {
        ...
        const CurrentChart = ()=>{
            ...
            return  <div className={`${className}-current-chart-all`}>
                        <Chart
                            ...
                            data = {data.option}
                            onClick = {(data)=>{
                                browserHistory.push({pathname: `/personal/${data.name}`,state:{backurl: `/departmentlist/${this.props.params.name}`,value: this.props.location.state.value}})
                            }}
                        />
                    </div>
        }
