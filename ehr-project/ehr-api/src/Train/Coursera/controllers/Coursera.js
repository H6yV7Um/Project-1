            ...
            for(let i = 2; i <= parseInt(worksheet['!ref'].replace(/[a-zA-Z]/g, '').split(':')[1]); i++)
            {
                if(worksheet[`A${i}`].v)
                {
                    ...
                    let records = `https://www.coursera.org/account/accomplishments/records/`,
                        ...
                        verity = `https://www.coursera.org/account/accomplishments/verify/`,
...
exports.searchCourseraByDepartment = async (ctx,next)=>
{
    ...
    else
    {
        ...
        let data = await Coursera.find({'user.department_ids':{"$all":[currentdepartment[0].department_id] }}, (err, docs) => {
            if(err) {
                console.log(err);
            }else {
                return docs.length == 0 ? null : docs
...
let asyloadimg = async(courseraInfo)=> {
    ...
    Coursera.findOne({ certificate: courseraInfo.certificate }, function (err, doc){
