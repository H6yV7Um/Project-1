...
class UsersChart extends Component {
        ...
        if(this.data() && !this.isUnmount)
        {
            ...
            this.data().map(okr => {
                ...
                okr.feedbacks.map(feedback => {
                    ...
                    option.series[key1].data[key2] = feedback.comment_info.score;
                })
            })

            option.grid.top = Math.ceil(option.legend.data.length * 75 / this.state.width) * 30;
    ...
    render() {
        let className = 'appComponentsOkrUsersChart';
