    ...
    if(usersInfo.status.code == 0)
    {
        ...
        for(let userInfo of usersInfo.data)
        {
            ...
            if (userInfo.department_ids.length === 0) {
                userInfo.department_ids.push(1)
            }
            // 更新成员信息
...
/**
 * 初始化申请审批人 (实际审批人不依赖此初始化,这只是作为一个初始)
 */
exports.initVerifier = async (ctx, next) => {
    await User.updateMany({user_id : {$in : CONFIG.VERIFY_USERS_ID}}, {$set : {status : 200}});
    await User.updateMany({user_id : {$nin : CONFIG.VERIFY_USERS_ID}, status : 200}, {status : 100});
    Response.success(ctx, CONFIG.VERIFY_USERS_ID);
}
