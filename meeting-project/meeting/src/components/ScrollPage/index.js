...
class ScrollPage extends Component {
    ...
    static propTypes =
    {
        ...
        // 总页数
        pageSum             :   PropTypes.number,
        // 每页宽度
        pageWidth           :   PropTypes.number,
        // 每页高度
        pageHeight          :   PropTypes.number,
        // 是否自切换
        isOwnChange         :   PropTypes.bool,
        // 是否单页切换
        isSinglePage        :   PropTypes.bool,
        // 是否从边缘滑入
        isEdge              :   PropTypes.bool,
        // 边缘有效值
        edgeValidNum        :   PropTypes.number,
        // 切换速度临界点
        changeSpeed         :   PropTypes.number,
        // 切换距离临界点
        changeDistance      :   PropTypes.number,
        // 切换最小距离
        changeMinDistance   :   PropTypes.number,
        // 上一页
        prev                :   PropTypes.func,
        // 下一页
        next                :   PropTypes.func,
        // 切换延迟 (ms)
        changeOffsetTime    :   PropTypes.number,
        // 切换时间 (ms)
        changeDuration      :   PropTypes.number,
        // 切换动画名
        changeAnimateName   :   PropTypes.oneOf(['linear', 'swing', 'easeInQuad', 'easeOutQuad', 'easeInOutQuad',
    ...
    move = e => {
        ...
        // 计算滑动方向
        if (!this.moveDirection && Math.pow(this.endCoordinate.y - this.startCoordinate.y, 2) +
            Math.pow(this.endCoordinate.x - this.startCoordinate.x, 2) >= 9) {
            // 正切
            const tan = (this.endCoordinate.y - this.startCoordinate.y) / (this.endCoordinate.x - this.startCoordinate.x)
