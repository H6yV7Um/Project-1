...
class TextList extends Component {
    static propTypes = {
        ...
        // 图标大小
        iconSize: PropTypes.oneOf(['lg', 'md', 'sm', 'x-sm']),
        // 字体大小
        fontSize: PropTypes.oneOf(['lg', 'md', 'sm', 'x-sm']),
    ...
    render () {
        ...
        if (this.props.extraContent.length > 0) {
            this.props.content.map((v, k) => {
                ...
                // 图标
                const icon = this.props.content.length > 1 && this.props.icon
                    ? <Icon type={this.props.icon} className={`${className}-icon ${className}-icon-${this.props.iconSize}`} /> : null
                // 虚线
                const line = this.props.isShowBottomLine === true && k < this.props.content.length - 1
                    ? <div className={`${className}-line`} /> : null
