...
class Upload extends Component {
    ...
    static propTypes = {
        ...
        fileType            :   PropTypes.oneOf(['image', 'music', 'video', 'file']),
        ...
        // 是否显示文件标签名
        isShowLabelName     :   PropTypes.bool,
        // 文件后缀
    ...
    initImageCut = (src, file) => {
        ...
        this.cutImage.image.onload = () => {
            ...
            // 裁剪内容宽度
            this.cutContentNum.width = document.body.clientWidth > 1000
                ? this.cutImage.width > 1000 ? document.body.clientWidth : 1000 : document.body.clientWidth

            // 裁剪内容高度
            this.cutContentNum.height = this.imageSize.height + (this.cutImage.height - this.imageSize.height > 150
                    ? 150 : this.cutImage.height - this.imageSize.height < 0 ? 0 : this.cutImage.height - this.imageSize.height)
    ...
    imageCutRegionMove = e => {
        ...
        cutRegionNum = {
            marginTop: cutRegionNum.marginTop <= 0 ? 0 : cutRegionNum.marginTop >= this.cutContentNum.height - this.imageSize.height
                ? this.cutContentNum.height - this.imageSize.height : cutRegionNum.marginTop,
            marginLeft: cutRegionNum.marginLeft <= 0 ? 0 : cutRegionNum.marginLeft >= this.cutContentNum.width - this.imageSize.width
                ? this.cutContentNum.width - this.imageSize.width : cutRegionNum.marginLeft
        }
            ...
            error: (XMLHttpRequest, textStatus, errorThrown) => {
                try {
                    if (XMLHttpRequest.responseText) {
                        Toast.info(<ToastContent type='fail' content={$.parseJSON(XMLHttpRequest.responseText).data.message} />, 5, null, false)
                    }
                } catch (e) {
    ...
    render () {
        ...
        const fileListMarginLeft = fileListLength < this.fileListStyle.length
            ? ($(this.fileListUl).width() + 30 - this.props.listSize.width * fileListLength) / (fileListLength + 1)
            : this.fileListStyle.marginLeft
        const fileUrl = []
            ...
            if (this.state.isUploading) {
                ...
                fileUploadContent =
                    <div className='file-upload-button'>
                        <input
                            type='file'
                            style={{width: this.props.listSize.width - 2, height: this.props.listSize.height - 2}}
