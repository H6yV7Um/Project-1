...
class Avatar extends Component {
    ...
    render()
    {
        ...
        if(avatar)
        {
            ...
            avatar.map((v, k) => {
                if(k < length)
                {
                    ...
                    if(v != '')
                    {
                        avatarDom = <span key={k} className={`avatar ${size}`} style={{backgroundImage : `url(${v + avatarSize})`}}></span>;
                    }
                    else
                    {
        ...
        {
            avatarRender = <Icon className={`avatar-default ${size}`} type="user-circle-o" />;
        }
