    ...
    render()
    {
        ...
        if(this.props.isLike || this.props.isReply || this.props.isForward || this.props.isDelete)
        {
            ...
            if(this.props.isLike)
            {
                ...
                overlay.push(<Popover.Item key={key} icon={icon}>{text}</Popover.Item>);
            }
            // 回复
            if(this.props.isReply)
            {
                overlay.push(<Popover.Item key="reply" icon={<Icon className={`${className}-operation-icon ${className}-operation-icon-reply`} type={'pinglun2'} classType={'it'} />}>
            ...
            {
                overlay.push(<Popover.Item key="delete" icon={<Icon className={`${className}-operation-icon ${className}-operation-icon-delete`} type={'remove'} />}>
